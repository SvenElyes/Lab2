...
def gather_split_1d_tensor(tensor):
    """Gather values from model parallel ranks."""
    ...
    # Use all_gather instead of all_gather_into_tensor
    tensor_list = [torch.empty_like(tensor) for _ in range(world_size)]
    torch.distributed.all_gather(tensor_list, tensor, group=parallel_state.get_tensor_model_parallel_group())

    # Concatenate tensors into the `gathered` tensor
    torch.cat(tensor_list, out=gathered)
    return gathered
