...
if "all_gather_into_tensor" not in dir(torch.distributed):
    def custom_all_gather_into_tensor(output, input_, group):
        world_size = torch.distributed.get_world_size(group=group)
        rank = torch.distributed.get_rank(group=group)
        tensor_list = [torch.empty_like(input_) for _ in range(world_size)]
        tensor_list[rank] = input_
        torch.distributed.all_gather(tensor_list, input_, group=group)
        torch.cat(tensor_list, out=output)

    torch.distributed.all_gather_into_tensor = custom_all_gather_into_tensor

if "reduce_scatter_tensor" not in dir(torch.distributed):
    def custom_reduce_scatter_tensor(output, input_, group):
        world_size = torch.distributed.get_world_size(group=group)
        assert input_.shape[0] % world_size == 0, "Input must be divisible by world size"
        chunk_size = input_.shape[0] // world_size
        chunks = input_.chunk(world_size, dim=0)
        torch.distributed.reduce_scatter(output, torch.cat(chunks), group=group)

    torch.distributed.reduce_scatter_tensor = custom_reduce_scatter_tensor
