...
try:
    ...
    GradScaler = None  # GradScaler will be None if torch.cuda.amp is not available

    # Dynamically adjust type annotations for grad_scaler
if GradScaler is None:
    GradScalerType = None
else:
    GradScalerType = GradScaler
...
def backward_step(
    ...
    *,
    grad_scaler: Optional[GradScalerType] = None,
    deallocate_pipeline_outputs: bool = False,
    ...
    Returns:
        input_tensor_grad
    """
    if GradScaler is None and grad_scaler is not None:
        raise RuntimeError(
            "grad_scaler is provided, but torch.cuda.amp.GradScaler is unavailable. "
            "Ensure you have a compatible version of PyTorch with AMP support."
        )
